...
#         '''
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            index  = np.argpartition(dists[i],self.k - 1)
            bincnt = np.bincount(self.train_y[index[:self.k]])
            if bincnt.shape[0] > 1:
                pred[i] = bincnt[0] < bincnt[1]
            else:
                pred[i] = 0
        return pred
    ...
    def predict_labels_multiclass(self, dists):
        ...
        for i in range(num_test):
            index = dists[i].argpartition(self.k - 1)
            bincount = np.bincount(self.train_y[index[:self.k]])
